...
function Nav() {
  ...
  return (
    ...
    <motion.nav className={`${open ? "block" : "hidden"} md:flex tracking-[2.7px] z-40 font-lightitems-center w-[65%] md:w-[60%] h-screen md:h-[96px] bg-white bg-opacity-[4%] backdrop-blur-3xl`}
                    ...
                    <Link className="md:h-full relative select-none" key={index} href={e.href}>
                        <motion.li
                            ...
                            <motion.div 
                                ...
                                className={`bg-white ${path === e.href ? "max-sm:h-full md:!w-full bg-opacity-100" : "bg-opacity-50"} absolute right-0 md:bottom-0 max-sm:w-1 md:h-1 md:![--w-from:0%] md:[--w-to:100%] self-end`}></motion.div>
                        </motion.li>
                    </Link>
                ))
            }
